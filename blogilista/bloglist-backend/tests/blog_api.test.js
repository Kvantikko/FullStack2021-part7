...
describe('when there is initially one user at db and login is succesfull', () => {
  ...
  beforeEach(async () => {   
    ...
    const string = 'bearer '
    token = string.concat(result.body.token)
  })
  ...
  test('deletion of a blog succeeds with status code 204 if id is valid', async () => {
    ...
    // ----- FIRST ADDING THE BLOG TO DELETE -----
    ...
    titles = blogsAtEnd.map(b => b.title)
  
    expect(titles).not.toContain(blogToDelete.title)
  })

  test('deletion of a blog is unsuccessful with status code 401 if request doesnt have token', async () => {
